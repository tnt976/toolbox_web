...
def generate_25digit_house_code(building_num, unit_num, floor_num, room_num):
    ...
    house_code = (district_code + street_code + community_code + 
                  building_code + unit_code + floor_code + room_code + check_digit)
...
def generate_19digit_building_code(building_num):
    ...
    return district_code + street_code + community_code + building_code + unit_code + floor_code


def regenerate_house_codes():
    """重新生成正确格式的房屋编码"""
    print("="*80)
    print("生成标准25位房屋编码")
    print("="*80)
    ...
    for i, house in enumerate(house_data[:10], 1):
        ...
        print(f"    房屋编码: {house_code} (长度: {len(house_code)} 位)")
